    ...
    return (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-in fade-in">
            ...
            >
                ...
                {/* Header */}
                <div className="px-4 md:px-8 py-6 border-b border-white/5 bg-zinc-900/30 flex justify-between items-center shrink-0 gap-4">
                    <div className="flex items-center gap-3 md:gap-4 min-w-0">
                        <div className="w-10 h-10 rounded-full bg-[#ff5500] flex items-center justify-center font-black text-white shrink-0">
                            {stage === 'uploading' ? <span className="animate-spin">⟳</span> : files.length}
                        </div>
                        <div className="min-w-0">
                            <h2 className="text-sm md:text-xl font-black text-white uppercase tracking-tight truncate leading-tight">
                                {stage === 'uploading' ? 'Uploading...' : 'Review & Release'}
                            </h2>
                            <p className="text-[10px] md:text-xs text-zinc-500 font-bold uppercase tracking-widest truncate">{files.length} Files Selected</p>
                        </div>
                    </div>
                    ...
                    <div className="flex bg-black/40 rounded-lg p-1 shrink-0">
                        <button
                            ...
                            className={`px-3 py-1.5 rounded-md text-[9px] md:text-xs font-bold uppercase tracking-wider transition-all ${uploadMode === 'individual' ? 'bg-white text-black shadow-lg' : 'text-zinc-500 hover:text-white'}`}
                        >
                            Song
                        </button>
                        <button
                            onClick={() => setUploadMode('versions')}
                            className={`px-3 py-1.5 rounded-md text-[9px] md:text-xs font-bold uppercase tracking-wider transition-all ${uploadMode === 'versions' ? 'bg-white text-black shadow-lg' : 'text-zinc-500 hover:text-white'}`}
                        >
                            Album
                        </button>
                    </div>
