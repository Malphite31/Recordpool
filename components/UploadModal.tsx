...
const UploadModal: React.FC<UploadModalProps> = ({ onClose, onUpload, defaultArtist = '', existingAlbums = [], defaultAlbum = '' }) => {
    ...
    return (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-0 md:p-4 bg-black/90 backdrop-blur-md animate-in fade-in">
            <div
                onDragOver={handleDragOver}
                onDragLeave={handleDragLeave}
                onDrop={handleDrop}
                className={`bg-[#0c0c0e] w-full h-full md:max-w-5xl md:h-[85vh] rounded-none md:rounded-[32px] border-x-0 md:border border-white/5 flex flex-col shadow-2xl relative overflow-hidden transition-all ${isDragOver ? 'border-[#ff5500] bg-[#ff5500]/5 scale-[1.01]' : ''}`}
            >

                {/* Header */}
                <div className="px-5 py-4 md:px-8 md:py-6 border-b border-white/5 bg-zinc-900/30 flex justify-between items-center shrink-0">
                    <div className="flex items-center gap-3 md:gap-4">
                        <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-[#ff5500] flex items-center justify-center font-black text-white text-xs md:text-base">
                            {stage === 'uploading' ? <span className="animate-spin">⟳</span> : files.length}
                        </div>
                        <div>
                            <h2 className="text-base md:text-xl font-black text-white uppercase tracking-tight whitespace-nowrap">
                                {stage === 'uploading' ? 'Uploading...' : 'Review & Release'}
                            </h2>
                            <p className="text-[10px] md:text-xs text-zinc-500 font-bold uppercase tracking-widest">{files.length} Files Selected</p>
                        </div>
                    </div>
                    {stage !== 'uploading' && (
